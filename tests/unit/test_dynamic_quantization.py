...
def test_classify_tensor_roles():
    # 1D tensors
    assert (
        classify_tensor_role("model.layers.0.input_layernorm.weight", (1024,))
        == TensorRole.NORMALIZATION_1D
    )
    ...
    # Attention Value & Out
    assert (
        classify_tensor_role("model.layers.0.self_attn.v_proj.weight", (1024, 1024))
        == TensorRole.ATTN_VALUE
    )
    assert (
        classify_tensor_role("model.layers.0.self_attn.o_proj.weight", (1024, 1024))
        == TensorRole.ATTN_OUT
    )

    # Attention Q & K
    assert (
        classify_tensor_role("model.layers.0.self_attn.q_proj.weight", (1024, 1024))
        == TensorRole.ATTN_QK
    )
    assert (
        classify_tensor_role("model.layers.0.self_attn.k_proj.weight", (1024, 1024))
        == TensorRole.ATTN_QK
    )

    # FFN Down & Gate/Up
    assert (
        classify_tensor_role("model.layers.0.mlp.down_proj.weight", (1024, 4096))
        == TensorRole.FFN_DOWN
    )
    assert (
        classify_tensor_role("model.layers.0.mlp.gate_proj.weight", (4096, 1024))
        == TensorRole.FFN_GATE_UP
    )
    assert (
        classify_tensor_role("model.layers.0.mlp.up_proj.weight", (4096, 1024))
        == TensorRole.FFN_GATE_UP
    )
...
def test_quantization_policy_resolution():
    ...
    # Embeddings -> F16
    assert (
        ud_policy.resolve_dtype("model.embed_tokens.weight", TensorRole.EMBEDDING, (151936, 1024))
        == DType.F16
    )

    # Output head -> Q8_0
    assert (
        ud_policy.resolve_dtype("lm_head.weight", TensorRole.OUTPUT_HEAD, (151936, 1024))
        == DType.Q8_0
    )

    # Attn Value & FFN Down -> Q8_0
    assert (
        ud_policy.resolve_dtype("v_proj.weight", TensorRole.ATTN_VALUE, (1024, 1024)) == DType.Q8_0
    )
    assert (
        ud_policy.resolve_dtype("down_proj.weight", TensorRole.FFN_DOWN, (1024, 4096)) == DType.Q8_0
    )

    # Attn QK & FFN Gate/Up -> Q4_0
    assert ud_policy.resolve_dtype("q_proj.weight", TensorRole.ATTN_QK, (1024, 1024)) == DType.Q4_0
    assert (
        ud_policy.resolve_dtype("gate_proj.weight", TensorRole.FFN_GATE_UP, (4096, 1024))
        == DType.Q4_0
    )
