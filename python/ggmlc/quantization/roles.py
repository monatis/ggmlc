...
class TensorRole(Enum):
    ...
    FFN_GATE_UP = (
        "ffn_gate_up"  # Feed-Forward gate & up projections (gate_proj, up_proj, w1, w3, fc1)
    )
...
def classify_tensor_role(name: str, dims: list[int] | tuple[int, ...] | None = None) -> TensorRole:
    ...
    if (
        ...
        and len([d for d in dims if d > 1]) <= 1
    ):
        return TensorRole.NORMALIZATION_1D
    ...
    if any(
        k in name_lower
        for k in ["v_proj", "attn_v", "value.weight", "w_v", "wv", "attention.v", "attn.v", "qkv"]
    ):
    ...
    if any(
        k in name_lower
        for k in ["o_proj", "out_proj", "attn_output", "wo", "w_o", "attention.o", "attn.out"]
    ):
