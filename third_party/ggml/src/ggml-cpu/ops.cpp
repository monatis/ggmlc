        ...
        case GGML_TYPE_I32:
            {
                ...
                else if (dst->type == GGML_TYPE_I32) ggml_compute_forward_dup_flt<int32_t, int32_t>(params, dst);
                else if (dst->type == GGML_TYPE_I64) ggml_compute_forward_dup_flt<int32_t, int64_t>(params, dst);
                else if (dst->type == GGML_TYPE_I16) ggml_compute_forward_dup_flt<int32_t, int16_t>(params, dst);
                else if (dst->type == GGML_TYPE_I8)  ggml_compute_forward_dup_flt<int32_t, int8_t>(params, dst);
                else GGML_ABORT("not implemented");
            } break;
        case GGML_TYPE_I16:
            {
                if (dst->type == GGML_TYPE_I16) ggml_compute_forward_dup_flt<int16_t, int16_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32) ggml_compute_forward_dup_flt<int16_t, float>(params, dst);
                else if (dst->type == GGML_TYPE_I32) ggml_compute_forward_dup_flt<int16_t, int32_t>(params, dst);
                else if (dst->type == GGML_TYPE_I64) ggml_compute_forward_dup_flt<int16_t, int64_t>(params, dst);
                else GGML_ABORT("not implemented");
            } break;
        case GGML_TYPE_I8:
            {
                if (dst->type == GGML_TYPE_I8) ggml_compute_forward_dup_flt<int8_t, int8_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32) ggml_compute_forward_dup_flt<int8_t, float>(params, dst);
                else if (dst->type == GGML_TYPE_I32) ggml_compute_forward_dup_flt<int8_t, int32_t>(params, dst);
                else if (dst->type == GGML_TYPE_I64) ggml_compute_forward_dup_flt<int8_t, int64_t>(params, dst);
                else GGML_ABORT("not implemented");
            } break;
        case GGML_TYPE_I64:
            {
                if (dst->type == GGML_TYPE_I64) ggml_compute_forward_dup_flt<int64_t, int64_t>(params, dst);
                else if (dst->type == GGML_TYPE_F32) ggml_compute_forward_dup_flt<int64_t, float>(params, dst);
                else if (dst->type == GGML_TYPE_I32) ggml_compute_forward_dup_flt<int64_t, int32_t>(params, dst);
                else if (dst->type == GGML_TYPE_I16) ggml_compute_forward_dup_flt<int64_t, int16_t>(params, dst);
                else if (dst->type == GGML_TYPE_I8)  ggml_compute_forward_dup_flt<int64_t, int8_t>(params, dst);
                else GGML_ABORT("not implemented");
            } break;
        default:
            {
...
static void ggml_compute_forward_repeat_generic(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
                    ...
                    for         (int i1 = 0; i1 < nr1;  i1++) {
                        for     (int k1 = 0; k1 < ne01; k1++) {
                            for (int i0 = 0; i0 < nr0;  i0++) {
                                ...
                                const char * x = (const char *) src0->data + (k3)*nb03 + (k2)*nb02 + (k1)*nb01;
                                memcpy(y, x, ne00 * type_size);
                            }
                        }
                    }
                }
...
void ggml_compute_forward_repeat(
    ...
    switch (src0->type) {
        ...
        default:
            {
                ggml_compute_forward_repeat_generic(params, dst);
            } break;
    }
}
