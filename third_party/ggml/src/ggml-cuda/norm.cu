...
static __global__ void add_rms_norm_mul_f32(
        const float * a, const float * b, const float * mul, float * add_dst, float * dst,
        const int ncols,
        const int64_t stride_a_row, const int64_t stride_a_channel, const int64_t stride_a_sample,
        const int64_t stride_b_row, const int64_t stride_b_channel, const int64_t stride_b_sample,
        const int64_t stride_mul_row, const int64_t stride_mul_channel, const int64_t stride_mul_sample,
        const float eps) {
    // Fused residual prologue for pre-norm Transformers:
    //   add_dst = a + b          (kept live — residual stream is reused)
    //   dst     = rms_norm(a+b) * w
    const int nrows     = gridDim.x;
    const int nchannels = gridDim.y;
    const int row       = blockIdx.x;
    ...
    a   += sample*stride_a_sample + channel*stride_a_channel + row*stride_a_row;
    b   += sample*stride_b_sample + channel*stride_b_channel + row*stride_b_row;
    mul += sample*stride_mul_sample + channel*stride_mul_channel + row*stride_mul_row;
    add_dst += ((sample*nchannels + channel)*nrows + row)*ncols;
    dst     += ((sample*nchannels + channel)*nrows + row)*ncols;
    ...
    for (int col = tid; col < ncols; col += block_size) {
        dst[col] = scale * add_dst[col] * mul[col];
    }
}

static void add_rms_norm_mul_f32_cuda(
        const float * a, const float * b, const float * mul, float * add_dst, float * dst,
    ...
    if (ncols < 1024) {
        const dim3 block_dims(256, 1, 1);
        const size_t smem = block_dims.x > WARP_SIZE ? 32 * sizeof(float) : 0;
        add_rms_norm_mul_f32<256><<<blocks_num, block_dims, smem, stream>>>(
            a, b, mul, add_dst, dst, ncols,
            stride_a_row, stride_a_channel, stride_a_sample,
            stride_b_row, stride_b_channel, stride_b_sample,
            stride_mul_row, stride_mul_channel, stride_mul_sample, eps);
    } else {
        const dim3 block_dims(1024, 1, 1);
        const size_t smem = block_dims.x > WARP_SIZE ? 32 * sizeof(float) : 0;
        add_rms_norm_mul_f32<1024><<<blocks_num, block_dims, smem, stream>>>(
            a, b, mul, add_dst, dst, ncols,
            stride_a_row, stride_a_channel, stride_a_sample,
            stride_b_row, stride_b_channel, stride_b_sample,
            stride_mul_row, stride_mul_channel, stride_mul_sample, eps);
...
void ggml_cuda_op_add_rms_norm_fused(ggml_backend_cuda_context & ctx,
                                     ...
                                     ggml_tensor *               mul_tensor) {
    GGML_ASSERT(add_tensor && rms_norm_tensor && mul_tensor);
    ...
    GGML_ASSERT(w->type == GGML_TYPE_F32 && mul_tensor->type == GGML_TYPE_F32);
    GGML_ASSERT(add_tensor->type == GGML_TYPE_F32 && rms_norm_tensor->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_are_same_shape(a, b));
    GGML_ASSERT(ggml_are_same_shape(a, add_tensor));
    GGML_ASSERT(ggml_is_contiguous_rows(a) && ggml_is_contiguous_rows(b));
    GGML_ASSERT(ggml_is_contiguous_rows(w));
    GGML_ASSERT(eps >= 0.0f);
    GGML_ASSERT(add_tensor->data && mul_tensor->data);
    ...
    GGML_ASSERT(a->nb[0] == ts && b->nb[0] == ts && w->nb[0] == ts);

    const int64_t mul_s01 = (w->ne[1] > 1) ? (int64_t)(w->nb[1] / ts) : 0;
    const int64_t mul_s02 = (w->ne[2] > 1) ? (int64_t)(w->nb[2] / ts) : 0;
    const int64_t mul_s03 = (w->ne[3] > 1) ? (int64_t)(w->nb[3] / ts) : 0;

    add_rms_norm_mul_f32_cuda(
        (const float *) a->data,
        (const float *) b->data,
        (const float *) w->data,
        (float *) add_tensor->data,
        (float *) mul_tensor->data,
        (int) ne00, (int) ne01, (int) ne02, (int) ne03,
        (int64_t)(a->nb[1] / ts), (int64_t)(a->nb[2] / ts), (int64_t)(a->nb[3] / ts),
        (int64_t)(b->nb[1] / ts), (int64_t)(b->nb[2] / ts), (int64_t)(b->nb[3] / ts),
