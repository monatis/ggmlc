...
void ggml_cuda_mul_mat_q_rms_norm_mul(
    ...
    GGML_ASSERT(ggml_is_contiguous(w) || w->nb[0] == sizeof(float));
    ...
    ggml_cuda_pool_alloc<float> row_scales(ctx.pool(), nrows);

    const int64_t s11 = src1->nb[1] / ts_src1;
    const int64_t s12 = src1->nb[2] / ts_src1;
    const int64_t s13 = src1->nb[3] / ts_src1;

    // Row-major token packing for scales: row = ((i3*ne12)+i2)*ne11 + i1
    // For typical [K, S, 1, 1] activations, stride_row = s11.
    rms_norm_row_scales_cuda(x_d, row_scales.get(), ne10, nrows, s11, eps, stream);
    quantize_mmq_q8_1_rms_mul_cuda(
        x_d, row_scales.get(), w_d, src1_q8_1.get(), src0->type,
    ...
    const mmq_args args = {
        ...
        nullptr,
        ne00, ne01, ne1, s01, ne11, s1,
        ne02, ne12, s02, s12q, s2,
        ne03, ne13, s03, s13q, s3,
        ne1};
    ggml_cuda_mul_mat_q_switch_type(ctx, args, stream);
}
