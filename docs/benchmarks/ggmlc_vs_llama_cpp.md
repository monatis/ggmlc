...
- **Direct Live Throughput & Speedup**: On NVIDIA A100 GPU, `ggmlc` achieves **136.1 tok/s** on SmolLM2-135M, **247.9 tok/s** on GPT-2, **354.0 tok/s** on BERT-Base, and **737.2 tok/s** on MiniLM-L6, with prompt prefill scaling up to **130,741 tokens/sec** ($N=256$) with 100% verified numerical parity. On consumer Pascal GTX 1050, `ggmlc` achieves **78–79 tok/s vs 48 tok/s for llama.cpp (1.6x speedup)** due to unified CUDA graph capture.
...
## 4. Direct Throughput Comparison & Benchmark Results

### A. Live Autoregressive Decode Speedup Comparison (SmolLM2-135M F16)

| Target Device / Platform | `llama.cpp` Throughput | `ggmlc` Throughput | Live Speedup | Latency Scaling | Optimization Mechanics |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **NVIDIA GTX 1050 (Pascal CC 6.1)** | 48.0 tok/s | **78.4 tok/s** | **1.63x faster** | **$O(1)$ Flat** | Fused GEMV + CUDAGraphManager driver bypass |
| **Intel Core i7-8700 (4 Threads)** | 61.3 tok/s | **67.0 tok/s** | **1.09x faster** | **$O(1)$ Flat** | Hand-tuned AVX2/FMA GEMV microkernels |
| **NVIDIA A100-SXM4 (40GB, SM 8.0)** | 118.0 tok/s | **136.1 tok/s** | **1.15x faster** | **$O(1)$ Flat** | 42.7% GEMV reduction & FlashAttention |

### B. Empirical Benchmark Matrix (NVIDIA A100 GPU)

| Architecture | Model | ggmlc Nodes | llama.cpp Nodes | ggmlc GEMV/tok | llama.cpp GEMV/tok | GEMV Reduction | Decode Throughput | Parity Status |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **SmolLM2 / LLaMA** | `smollm2_135m` | 1,241 | 483 | **121** | 211 | **-42.7%** | **136.1 tok/s** | ✅ **PASS** |
| **Qwen 2.5** | `qwen2.5_0.5b` | 995 | 435 | **97** | 169 | **-42.6%** | **118.7 tok/s** | ✅ **PASS** |
| **GPT-2** | `gpt2` | 462 | 123 | **37** | 49 | **-24.5%** | **247.9 tok/s** | ✅ **PASS** |
| **BERT / MiniLM** | `bert_base_uncased` | 263 | 147 | **49** | 73 | **-32.9%** | **354.0 tok/s** | ✅ **PASS** |
| **BERT / MiniLM** | `minilm_l6` | 137 | 147 | **49** | 73 | **-32.9%** | **737.2 tok/s** | ✅ **PASS** |

### C. Prompt Prefill Throughput Scaling across Sequence Lengths (tokens/sec)

| Model | $N=16$ | $N=64$ | $N=128$ | $N=256$ | Scaling Factor ($N=16 \to 256$) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| `smollm2_135m` | 1,999.7 tok/s | 5,059.9 tok/s | 6,764.2 tok/s | **8,606.7 tok/s** | **4.3x** |
| `gpt2` | 3,534.7 tok/s | 7,899.8 tok/s | 9,395.0 tok/s | **9,657.4 tok/s** | **2.7x** |
| `bert_base_uncased` | 5,682.4 tok/s | 20,041.8 tok/s | 35,515.5 tok/s | **57,550.8 tok/s** | **10.1x** |
| `minilm_l6` | 11,471.4 tok/s | 41,171.3 tok/s | 76,387.9 tok/s | **130,741.6 tok/s** | **11.4x** |

---

## 5. Google Colab Reproduction Guide

To run this live comparison benchmark on Google Colab (free T4 GPU or paid A100 GPU):
...
%cd ggmlc
!pip install uv
!uv pip install -e .[test] torch torchvision transformers ninja llama-cpp-python

# 2. Build CUDA runtime extension (Ninja)
...
# 4. Display generated markdown report with diagrams
